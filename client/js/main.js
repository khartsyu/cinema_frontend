document.addEventListener('DOMContentLoaded', async () => {
    ...
    function renderMovies() {
        ...
        moviesList.innerHTML = allData.films.map(film => {
            const filmSeances = allData.seances.filter(seance => seance.seance_filmid === film.id);
            ...
            for (const [hallName, seances] of Object.entries(sessionsByHall)) {
                const timeButtonsHtml = seances.map(seance => `
                    <li>
                        <button class="movie__time-btn" data-session-id="${seance.id}" data-session-time="${seance.seance_time}" data-film-id="${film.id}" data-hall-id="${seance.seance_hallid}">
                            ${seance.seance_time}
                        </button>
                    </li>
                `).join('');
